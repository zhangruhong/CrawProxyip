...
public class Crawl implements Runnable {
	 ...
	 *            是否验证 true 验证代理
	 */
	public Crawl(ProxyIPDao dao, String[] urllist, String[] ipList, String SupplierName, boolean isValid, int waitmin) {
	...
	public void run() {
		...
		while (true) {
			...
			if (!issucc) {
				SendInfo.sentMessage("18020280367", SupplierName + "供应商代理抓取不到了!");
				...
				if (isCheck) {
					...
					if (null == content) {
						...
						if (flag2) {
							...
							if (dao.checkProxyExits(proxyip, Integer.parseInt(port))) {
								dao.insertProxyIPData(proxyip, Integer.parseInt(port));
							} else {
								dao.updateProxyIPData(proxyip, Integer.parseInt(port));
							}
						}
					} else {
						System.out.println("代理库中已经存在该代理ip..." + SupplierName);
					}
				} else {
